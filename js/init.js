...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (desktopFolder && desktopFolder.children) {
    Object.keys(desktopFolder.children).forEach(appName => {
      ...
      const gameNames = ['Snake', 'Minesweeper', 'Plants vs Zombies', 'Minecraft', 'Roblox', 'Mario', 'Progressbar', 'Hydra App', 'Pinball'];
      ...
      if (appData.type === "file") {
        ...
        const programName = appData.program || appName;
        // Check if it's a game
        const gameNames = ['Snake', 'Minesweeper', 'Plants vs Zombies', 'Minecraft', 'Roblox', 'Mario', 'Progressbar', 'Hydra App', 'Pinball'];
        if (gameNames.includes(programName)) {
      ...
      smItem.innerHTML = `
        <img src="${getIcon(targetPath)}" alt="${appName} icon" width="32" height="32">
        <span>${appName}</span>
      `;
  ...
  secondItems.forEach(item => {
    ...
    smItem.innerHTML = `
      <img src="${getIcon(item.program || item.name)}" alt="${item.name} icon" width="32" height="32">
      <span>${item.name}</span>
    `;
    smItem.addEventListener('click', () => {
      const path = smItem.getAttribute('data-path');
  ...
  horrorModeItem.style.marginTop = '20px'; // Add some space above
